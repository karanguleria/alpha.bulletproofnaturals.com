            ...
            <div class="grid gap-4 sm:gap-4 sm:grid-cols-3 px-4 sm:px-0">
                <div class="col-span-1 sm:col-span-2">
                    ...
                    <div class="inline-block mt-2 -mx-1 pr-2 w-1/3">
                        <select id="province" name="state" class="block w-full px-2 py-2 bg-white border border-black-300 rounded-md text-sm sm:text-md shadow-sm placeholder:text-slate-600 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-black-500 invalid:text-primary-600 focus:invalid:border-primary-500 focus:invalid:ring-primary-500">
                            ...
                            <option {{ old('state') == $key ? 'selected' : '' }} value="{{$key}}">{{$state_name}}</option>
                            @endforeach
                        </select>
                        @error('state')
                    ...
                    {{--<div id="diff_address" class="different_info mt-4">
                        <input type="checkbox" id="different_check" name="different" class="w-4 h-4 text-primary-600 bg-gray-100 rounded border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 " /> <span class="pl-3 mt-2 text-gray-800 font-black" onclick="myFunction()">Different Shipping Address</span>
                ...
                <div class="col-span-1">
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        <thead class="text-sm sm:text-md text-gray-700 uppercase">
                            <tr>
                                <th colspan="2" scope="col" class="py-2 px-2">
                                    Product image
                                </th>

                                <th scope="col" class="py-2 px-2 text-center">
                                    QUANTITY
                                </th>
                        ...
                        <tbody>
                            <tr class="bg-white ">
                                ...
                                <td class="py-2 px-2">
                                    <h3 class="text-dark text-sm sm:text-md font-semibold">{{ $item->options->variant_name }} Pack - {{ $item->options->varient_tablets }} Tablets Only ${{ number_format(preg_replace('/\s+/', '', $item->subtotal), 2, '.', ',') }}
                                </td>
                                <td class="py-2 px-2">
                                    <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                                        <button type="button" class="border border-gray-200 inline-block px-6 py-2.5 bg-white text-dark font-medium text-sm sm:text-md leading-tight uppercase transition duration-150 ease-in-out">{{ $item->qty }}</button>
                                    </div>
                                </td>
                                <td class="py-2 px-2">
                    ...
                    <div class="mt-4 update_information">
                        <button class="update_info w-full px-6 py-3 bg-dark text-white font-md text-sm sm:text-md uppercase rounded-3xl shadow-md hover:bg-primary-500 hover:shadow-lg focus:bg-primary-500 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-primary-500 active:shadow-lg transition duration-150 ease-in-out" id="payment_now" type="submit">Continue Payment</button>
                        @php
                        $url = '';
                        @endphp
                        <x-paddle-button :url="$url" class="px-6 py-3 bg-dark text-white font-md text-sm sm:text-md uppercase rounded-3xl shadow-md hover:bg-primary-500 hover:shadow-lg focus:bg-primary-500 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-primary-500 active:shadow-lg transition duration-150 ease-in-out text-center">Pay Now</x-paddle-button>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function myFunction() {
        $('#different_check').trigger('click');
    }

    $(document).ready(function() {
        $('.paddle_button').css('display', 'none');
        $('#different_check').prop('checked', false);
        $('#different_check').click(function() {
            if ($(this).prop("checked") == true) {
                $('.unchk').prop('required', true);
                console.log("checked");
                $(this).val('1');
                $(".different_address").slideDown();
            } else if ($(this).prop("checked") == false) {
                $('.unchk').prop('required', false);
                $(this).val('');
                $('.unchk').val('');
                console.log("checked");
